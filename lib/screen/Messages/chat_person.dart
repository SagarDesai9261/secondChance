...
//  late Timer _timer;
  ...
  Stream<List<ChatPerson>> _getChattedUsersStream() async* {
    ...
    final chatStream = _firestore.collection("HomeLessMembers").doc(userId).snapshots();
    ...
    await for (var documentSnapshot in chatStream) {
      ...
      for (final userDocId in userChats) {
        ...
        if (userDoc.exists) {
          ...
          final userName = userDoc['organizationName'] as String;
          final image = userDoc["images"][0] as String;
          ...
          final userDoc = await _firestore.collection('donor').doc(userDocId).get();
          if (userDoc.exists) {
            final chatId = userDoc.id;
            final userName = userDoc['fullName'] as String;
            final image = userDoc["image"] as String;
            ...
            final userDoc = await _firestore.collection('merchants').doc(userDocId).get();
            if (userDoc.exists) {
              final chatId = userDoc.id;
              final userName = userDoc['BusinessName'] as String;
              final image = userDoc["image"] as String;
